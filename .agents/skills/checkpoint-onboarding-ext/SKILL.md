...
> 与 Claude `agent-kit/claude/skills/checkpoint/SKILL.md` 行为一致。
...
- 真实 `.env`、`*.key`、私钥、token、cookie、credential、auth cache 不得进入 Git。
- Secret gate 应按路径片段/扩展名匹配, 避免把普通词（如 `Turkey.md`）误判为 `key`。
- 若 `git ls-files` 命中疑似 secret payload, fail-closed: 停止提交、提示 remove-from-index / 轮换。
- 例外: 路径精确匹配 `agent-kit/secrets/allowlist.yaml` 中 `classification: tracked-exception` 且 `risk_acceptance: user-approved` 的条目时可继续; 报告只写路径和批准状态, 不读取/打印内容。
...
## Step 7.1 — Jeff1993 文档仓 Commit (条件, 新增)
