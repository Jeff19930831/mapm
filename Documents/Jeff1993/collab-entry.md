...
> v3.3 改动: 上下文优化 — `onboarding-compact.md` 替代完整版作为每次对话加载入口；con-dev skill 精准读取（handoff 分层、plan 只读 Active、README 只读架构段）。
...
skill 会自动完成全部五阶段：`git pull` → 解析 YAML → clone 代码仓库 → **精准读取** handoff/plan → 环境检测 → 输出就绪报告。
