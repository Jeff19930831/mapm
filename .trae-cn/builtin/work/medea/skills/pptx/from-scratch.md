# Creating Presentations from Scratch (HTML-First Workflow)
...
**⚠️ CRITICAL: You MUST use Web Fonts (Google Fonts or @font-face)**

System fonts (e.g., Microsoft YaHei, PingFang SC, SimSun) are FORBIDDEN because:
- They cannot be embedded in PPTX files
- CJK characters will appear garbled on other computers
- Fonts will fall back to Arial, breaking the design

Choose ONE font style and use it consistently throughout:

| Style | Title Font | Body Font | Google Fonts URL |
|-------|------------|-----------|------------------|
| Sans-serif (Chinese) | Noto Sans SC | Noto Sans SC | `family=Noto+Sans+SC:wght@300;400;500;700` |
| Serif (Chinese) | Noto Serif SC | Noto Serif SC | `family=Noto+Serif+SC:wght@400;700` |
| Sans-serif (English) | Inter | Inter | `family=Inter:wght@300;400;500;700` |
| Serif (English) | Lora | Lora | `family=Lora:wght@400;700` |
...
Create an HTML file with slides that follow your design philosophy.
...
### Reference: PptxGenJS API

For advanced cases requiring fine-grained control, see [pptxgenjs-reference.md](pptxgenjs-reference.md). However, **you should still create HTML first** and only use PptxGenJS for specific elements that cannot be converted automatically.
...
1. **Always use `preconnect`** for Google Fonts to reduce latency
2. **Include fallback fonts** in your font stack (e.g., `'Noto Sans SC', 'Microsoft YaHei', sans-serif`)
3. **Use `font-display: swap`** to prevent invisible text during font loading
4. **Preload critical fonts** if needed:

```html
<link rel="preload" href="https://fonts.gstatic.com/s/notosanssc/v36/..." as="font" type="font/woff2" crossorigin>
```
...
```bash
# For HTML to PPTX conversion (install in dom-to-pptx directory)
cd skills/pptx/dom-to-pptx
npm install
