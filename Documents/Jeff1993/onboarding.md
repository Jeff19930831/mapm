...
2. 执行 Doc Lifecycle Gate：入口短、历史归档、重复问题 runbook 化。
2a. 更新 `plan.md` Active / Next / Archived Plans、`progress/YYYY-MM.md` 或 `progress.md`、`handoff.md` Persistent / Current Takeover / Index。
2b. 若本次工作引入新的常驻服务/进程/集成，同步更新 `handoff.md` Persistent Context 和对应 `docs/runbooks/<name>.md`（跨项目通用规程才放 `docs/sop/`）。
2c. 若本次工作新增/修改长期配置方法、服务器接入、密钥引用或恢复步骤，同步更新项目 `docs/runbooks/*.md` 或 `agent-kit/secrets/allowlist.yaml`；不得把 secret payload 写入 Git。
2d. 若本次涉及联动契约变更，执行项目联动协同检查：识别上游/下游，更新 contract/runbook，运行最小联动验证；未完成则不得标记完成。
...
4. ZCode 额外需要同步到 `~/.zcode/skills/<name>/SKILL.md`（ZCode 官方 skill 发现路径，通过设置 UI 或文件系统管理）。
5. 重启目标客户端后验证会话 Skills 列表是否出现该 skill。
6. 若未出现，再同步到客户端 fallback 目录（例如 Codex 的 `~/.codex/skills/<name>/SKILL.md`）并重启验证。
...
- **reference not payload**：可以同步 secret 名称、用途、存放位置、恢复步骤和校验命令；不得同步密钥值、私钥正文、token、cookie、session dump。
...
4. 运行 secret gate，确认真实 payload 没有被 Git tracked。
5. 在 `progress.md` 记录 `[RUNBOOK]` 或 `[SECRET-REF]` 变更事实和验证证据。

### 15.6 迁移提醒

若发现已 tracked 的疑似 secret 文件（如 `*key*`、`*token*`、`credentials*`），不要读取内容；先判断是否是 payload。若是 payload，执行 fail-closed：停止提交、移出 Git 跟踪、轮换密钥，并在 `progress.md` 记录处理结果。
